...
function QuasiDefiniteLDL!{F}(front::Front{F})
    # Recurse on the children
    num_children = length(front.children)
    for c=1:num_children
        QuasiDefiniteLDL!(front.children[c])
    end
	...
	#Since we don't have a weighted rank-k update we need to make a copy of the 
	#sub-diagonal block.
	FBL_COPY = copy(FBL)
   	
	#LDLT on the dense block 	
	QuasiDefinite.qdtf2!(string(:L)[1],FTL) #Call the quasi definite factorization 
	

	#Form AL^{-T}D^{-1}
    BLAS.trsm!('R','U','N','N',1.,FTL,FBL) #Unit diagonal L stored in the lower triangular
										   #part of FTL  
	#Form ALD{-1}
    BLAS.trsm!('R','L','T','U',1.,FTL,FBL_COPY) #Unit diagonal L stored in the lower triangular
										   #part of FTL 
    
	#Form AL^{-1}D^{-1}LA'
	BLAS.gemm!('N','T',1.,FBL,FBL_COPY,0.,FBR)
end
